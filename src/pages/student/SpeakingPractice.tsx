...
import { mockSpeakingExercises, mockSpeakingResults } from '@/data/mockData';
import { toast } from 'sonner';
import type { SpeakingExercise, SpeakingExerciseType } from '@/types';
...
export default function SpeakingPractice() {
  ...
  const filteredExercises = mockSpeakingExercises.filter(ex => {
  ...
  const startRecording = async () => {
    try {
      ...
      mediaRecorder.onstop = () => {
        ...
        setTimeout(() => {
          if (selectedExercise?.targetText) {
            setTranscript(selectedExercise.targetText.slice(0, -20)); // Simulate partial match
          } else {
            setTranscript('This is a sample transcription of what you said during the speaking practice session.');
          }
          setShowResults(true);
        }, 500);
      };
      ...
      {!selectedExercise ? (
        <>
          ...
          <Card>
            ...
            <CardContent>
              {mockSpeakingResults.length > 0 ? (
                <div className="space-y-3">
                  {mockSpeakingResults.map((result) => (
