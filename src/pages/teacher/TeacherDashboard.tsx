...
import { mockClasses, mockLessons } from '@/data/mockData';

export default function TeacherDashboard() {
  const totalStudents = mockClasses.reduce((acc, c) => acc + c.students.length, 0);
  const recentSubmissions = [
    { student: 'John Student', assignment: 'Grammar Quiz', submittedAt: '2 hours ago', status: 'pending' },
    { student: 'Emma Wilson', assignment: 'Speaking Exercise', submittedAt: '5 hours ago', status: 'graded' },
  ];
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground">Lessons Uploaded</p>
                <p className="text-3xl font-bold">{mockLessons.length}</p>
              </div>
                ...
                <div key={cls.id} className="flex items-center justify-between p-4 bg-muted rounded-lg">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{cls.students.length} students enrolled</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="w-24 h-2 bg-muted-foreground/20 rounded-full overflow-hidden mb-1">
                      <div className="h-full bg-primary rounded-full" style={{ width: '65%' }} />
                    </div>
                    <p className="text-xs text-muted-foreground">65% avg. completion</p>
                  </div>
                </div>
              ))}
            </div>
              ...
              <div key={index} className="flex items-center justify-between p-4 bg-muted rounded-lg">
                <div className="flex items-center space-x-4">
                  <div className="h-10 w-10 bg-primary/10 rounded-full flex items-center justify-center">
                    <span className="text-sm font-medium">{submission.student.charAt(0)}</span>
                  </div>
                  <div>
            ...
            {mockLessons.slice(0, 3).map((lesson) => (
              <div key={lesson.id} className="group relative bg-muted rounded-lg overflow-hidden">
                ...
                <div className="absolute bottom-3 left-3 right-3">
                  <Badge className="mb-1" variant="secondary">{lesson.category}</Badge>
                  <p className="text-white font-medium text-sm line-clamp-1">{lesson.title}</p>
                  <p className="text-white/70 text-xs">{lesson.duration} min • {lesson.viewCount || 234} views</p>
                </div>
              </div>
            ))}
          </div>
